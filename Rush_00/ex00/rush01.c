...
/*   Updated: 2022/10/16 19:15:13 by tlukanie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_slashes(int x, int y)
{
	if (x == 1)
	{
		if (y == 1)
...
void	print_stars_spaces(int a, int b, int c, int d)
{
	ft_putchar('*');
	while (c <= (a - 1) && a > 2)
	{
	...
	if (error_handler(x, y))
	{
		...
		while (check_y <= y)
		{
			...
			if ((check_x == 1 && check_y == 1) || (check_x == x && check_y == x)
				...
				print_stars_spaces(x, y, check_x, check_y);
			ft_putchar('\n');
			check_y++;
		}
	}
